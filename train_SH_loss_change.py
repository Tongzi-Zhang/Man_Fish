...
if __name__ == "__main__":
    ...
    # 增加计数loss，实现替换
    count_loss = nn.L1Loss(reduction='sum').to(device)
    ...
    # 动态调整学习率
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=20, eta_min=1e-6) #减少T_max，原来100，改为20
    ...
    start_epoch=0
    patience = 100  # Number of epochs to wait before switching loss
    no_improvement_epochs = 0  # Counter for epochs with no improvement
    use_counting_loss = False  # Flag to switch to counting loss
    switch_epochs = 10
    switch_count = 0
    for epoch in range(start_epoch, 2000):
        ...
        for i, (img, gt_dmap) in enumerate(val_dataloader):
            ...
            if use_counting_loss:
                val_loss = count_loss(et_dmap.sum(),(gt_dmap.sum()+1))
            else:
        ...
        if mae / len(val_dataloader) < min_mae:
            ...
            print(f"Min MAE: {min_mae:.4f} at epoch {min_epoch}, No improvement for {no_improvement_epochs} epochs")
        # Switch to counting loss if no improvement for 'patience' epochs
        if no_improvement_epochs >= patience and not use_counting_loss:
            print("Switching to counting loss...")
            use_counting_loss = True
            switch_count = 0
        if use_counting_loss:
            switch_count += 1
            if switch_count >= switch_epochs:
                print(f"Switching back to density loss at epoch{epoch}")
